...
class RAFTExhaustiveDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if img_name1 not in self.sample_weights:
            print(f"🚨 Missing Key in sample_weights: {img_name1}")
            print(f"✅ Available Keys: {list(self.sample_weights.keys())[:10]}")  # Show 10 sample keys
            raise KeyError(f"🚨 Missing Image Key: {img_name1}")
